...
@implementation ChampionSkinDictionary

+ (NSArray *) championList{
    //Takes champion dictionary keys and sorts them into alphbetized array.
    return [[[ChampionSkinDictionary championSkinDictionary] allKeys] sortedArrayUsingSelector:@selector(compare:)];
}

+ (NSArray *) skinListForChampion:(NSString *)championName {
    NSDictionary *champion = [[ChampionSkinDictionary championSkinDictionary] objectForKey:championName];
    NSArray *skinList = [champion objectForKey:@"skins"];
    NSMutableArray *skinNames = [NSMutableArray new];
    
    for (int i = 0; i < skinList.count; i++) {
        [skinNames addObject:[skinList[i] objectForKey:@"name"]];
    }
    return skinNames;
}
